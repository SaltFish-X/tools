    ...
    function buildCrossClassActionSetupLines(options, sequence, cls) {
      ...
      for (var i = 0; i < crossClass.length; i++) {
        ...
        if (info) {
          ...
          lines.push('/echo 错误！未知工序： ' + action + '\n');
        }
      }

      if (lines.length > 0) {
        lines.push('/echo 额外技能已装载完毕！ ' + soundEffect(options.stepSoundEffect, options.stepSoundEnabled) + '\n');
      }
    ...
    function buildSequenceLines(options, sequence, buffs) {
      ...
      for (var i = 0; i < sequence.length; i++) {
        ...
        if (info) {
          ...
          lines.push({text: '/echo 错误！未知工序： ' + action, time: 0});
        }
      }
    ...
    function buildMacroList(options, lines) {
      ...
      for (var j = 0; j < lines.length; j++) {
        ...
        if (macroLineCount === MAX_LINES - 1) {
          if (lines.length - (j + 1) > 1) {
            macroString += '/echo 宏 #' + macroIndex + ' 已执行完毕！ ' + soundEffect(options.stepSoundEffect, options.stepSoundEnabled) + '\n';
      ...
      if (macroLineCount > 0) {
        if (macroLineCount < MAX_LINES) {
          macroString += '/echo 宏 #' + macroIndex + ' 已执行完毕！ ' + soundEffect(options.finishSoundEffect, options.stepSoundEnabled) + '\n';
        }
        macroList.push({text: macroString, time: macroTime});
      }
